...
Same logic as `auto-shift.js`, restructured for Workers. Entry points in `src/index.js`: `fetch` (UI at `/`; JSON API under `/api/*` gated by the `UI_PIN` secret; **`/cron` external trigger** gated by the `CRON_KEY` secret → `runAuto`) and `scheduled` (Cloudflare cron → `runAuto`). `src/shift.js` is the Supabase client; `src/ui.js` is the single-page UI string — a **layered, animated Japanese night theme**. Fixed scene built as **4 parallax depth layers** (`.layer[data-depth]`, shifted by a rAF-smoothed `pointermove` handler): far (gradient + twinkling starfield + moon), mountains (ridge + Mt. Fuji), mid (pagoda + torii silhouettes), near (swaying lanterns, tree silhouettes, koi pond). A separate fixed `.fx` layer floats sakura petals + glowing fireflies. Frosted-glass cards float on top with a hover-lift, an entrance `reveal` stagger, and an amber `glow` ring on the currently-open shift. Palette: deep navy/charcoal base, **gold `--gold` as the primary accent**, **jade `--jade` as the highlight** (used for the "done" badge, the check-out button, the ×-multiplier figure). Discipline kept: one locked dark theme, locked radius scale (cards 18 / buttons 12 / pills), glass done right (no neon), animated primary button (shimmer), font **Be Vietnam Pro** (Vietnamese-native, not Inter), WCAG-AA contrast, **zero em-dashes**, and `prefers-reduced-motion` (kills parallax + all loops) / `prefers-reduced-transparency` (solid cards) fallbacks. Desktop-first, responsive (`.grid` 2-col ≥820px → 1-col mobile). Vanilla CSS/JS, no framework; Google Fonts via `<link>` (the only option in a single-string Worker).
...
- **OT hour stats + pay:** `/api/status` returns an `ot` summary from `getOtStats()` (`shift.js`) — sums approved `ot_requests` (covers + plain extra shifts) by `slotHours()`, applying a pay **multiplier per shift**: weekend (Sat/Sun by `shift_date`) **×2**, weekday **×1.5**; **money = multHours × `OT_RATE` (80 000 VND/h)**. Shape: `{ rate, thisMonth, month:{count,rawHours,multHours,money}, all:{…} }` (both a this-month bucket and an all-time bucket). The UI renders it as a standalone **Japanese pixel-art block** (`ui.js`: animated waterfall, Mt. Fuji, pine/sakura trees, meadow; "Press Start 2P" font for numbers) with a **filter** (Tháng này default / Tất cả all-time) showing raw hours, ×-multiplier hours, and money. Read-only mirror of the upstream app's "OT request" tab (we can't modify that app).
