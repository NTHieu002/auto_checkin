...
- **OT hour stats:** `/api/status` also returns an `ot` summary from `getOtStats()` (`shift.js`) — sums approved `ot_requests` (covers + plain extra shifts) by `slotHours()`, giving `{ totalHours, totalCount, thisMonthHours, byMonth[] }`. The UI shows it in a "Giờ OT của tôi" card. Read-only mirror of the app's "OT request" tab (we can't modify that upstream app).
