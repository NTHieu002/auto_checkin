...
Same logic as `auto-shift.js`, restructured for Workers. Entry points in `src/index.js`: `fetch` (UI at `/`; JSON API under `/api/*` gated by the `UI_PIN` secret; **`/cron` external trigger** gated by the `CRON_KEY` secret → `runAuto`) and `scheduled` (Cloudflare cron → `runAuto`). `src/shift.js` is the Supabase client; `src/ui.js` is the single-page UI string — a **daytime Mt. Fuji + sakura photo theme**: a full-viewport fixed background photo (cover) with a top+bottom darkening scrim, falling sakura petals overlay, and translucent blurred cards floating on top. The photo is `src/bg.js` (`export const BG = "data:image/jpeg;base64,…"`, a ~88 KB JPEG, imported into the CSS via `url(${BG})` — interpolated server-side, NOT in client JS). To change the background: regenerate `bg.js` (resize/compress an image to a small JPEG, base64 it into that one export). Desktop-first, responsive (`.grid` is 2-col ≥780px — shifts+controls | OT — collapsing to 1-col on mobile). "Press Start 2P" pixel font for the logo + numbers only (it lacks Vietnamese glyphs, so body text stays in a sans).
