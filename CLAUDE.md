...
Same logic as `auto-shift.js`, restructured for Workers. Entry points in `src/index.js`: `fetch` (UI at `/`; JSON API under `/api/*` gated by the `UI_PIN` secret; **`/cron` external trigger** gated by the `CRON_KEY` secret → `runAuto`) and `scheduled` (Cloudflare cron → `runAuto`). `src/shift.js` is the Supabase client; `src/ui.js` is the single-page UI string — a **refined Japanese night theme** (full-viewport fixed scene: indigo gradient + starfield, moon, Mt. Fuji + torii silhouettes, swaying lanterns, hill/tree silhouettes, a still koi pond, falling sakura petals; frosted-glass cards floating on top). Built to the `design-taste-frontend` skill's discipline: ONE dark theme locked, ONE decorative accent (lantern amber `--accent`) with green/neutral used only for semantic states, ONE radius scale (cards 16 / buttons 12 / pills), glass done right (inner highlight + tinted shadow, no neon), font **Be Vietnam Pro** (Vietnamese-native, deliberately not Inter; no pixel font), WCAG-AA button contrast, **zero em-dashes**, and `prefers-reduced-motion` / `prefers-reduced-transparency` fallbacks. Desktop-first, responsive (`.grid` 2-col ≥820px — shifts+controls | OT — 1-col on mobile). Vanilla CSS/JS, no framework (so the skill's React/Tailwind/Motion defaults don't apply); Google Fonts via `<link>` is the only option in a single-string Worker.
